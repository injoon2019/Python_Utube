...
#화면 타이틀 설정
pygame.display.set_caption("Injun Pang") #게임이름
...
stage_height = stage_size[1] #스테이지의 높이위에 캐릭터 위치 시키기 위해
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                weapon_x_pos = character_x_pos + (character_width/2) - (weapon_width/2)  #무기는 케릭터의 딱 중앙에서 발사되도록 한다.
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        else: #그 외에는 속도가 증감으로 가속도를 냄으로서 포물선 효과를 낼 수 있음.
            ball_val["to_y"] += 0.5
